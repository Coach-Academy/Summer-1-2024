...
signed main() {
    khaled
    // give an array of n elements N up to 1e6 , q queries of type what is the min of the elements [0 , idx]
    int n;cin>>n;
    for(int i = 0;i < n;i++)cin>>arr[i];
    for(int i = 0;i < n;i++)prefix[i] = arr[i];
    for(int i = 1;i < n;i++)prefix[i] = min(prefix[i - 1] , prefix[i]);
    int q;cin>>q;
    while(q--) {
        int idx;cin>>idx;idx--;
        cout<<prefix[idx]<<line;
    }
}
