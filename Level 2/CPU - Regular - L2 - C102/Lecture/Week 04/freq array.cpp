...
signed main() {
    khaled
    // given an array of size N: 0 < N < 1e6 ,for each value in the array >=-1e6 && <= 1e6, i want to ask about something: given x print yes if it is in the list otherwise no
    int n;cin>>n;
    for(int i = 0;i < n;i++) {
        cin>>v[i];
        freq[v[i] + (int)1e6]++;
