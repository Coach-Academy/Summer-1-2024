...
signed main() {
    khaled
    // give an array of n elements N up to 1e6 , q queries of type what is the summation of the elements [l , r]
    int n;cin>>n;
    for(int i = 0;i < n;i++)cin>>arr[i];
    for(int i = 0;i < n;i++)suf[i] = arr[i];
    for(int i = n - 2;i >= 0;--i)suf[i] += suf[i + 1];
    int q;cin>>q;
    while(q--) {
        int l , r;cin>>l>>r;
        l-- , r--;
        // if(l == 0)cout<<prefix[r]<<line;
        cout<<suf[l] - suf[r + 1]<<line;
    }
}
